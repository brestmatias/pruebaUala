...
import (
	...
	"gitlab.com/devskiller-tasks/rest-api-blog-golang/utils"
	"net/http"
	"strconv"
)
...
func handleGetPostByPostId(svc *RestApiService) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		var data []byte
		id := utils.GetFirstParam("/api/get/post/(.*)", r.URL.Path)
		if intId, err := strconv.ParseUint(id, 10, 64); err != nil {
			data, _ = json.Marshal(&AckJsonResponse{Message: fmt.Sprintf("wrong id path variable: %v", id), Status: http.StatusBadRequest})
			w.WriteHeader(http.StatusBadRequest)
		} else {
			if result, _ := svc.postRepository.GetById(intId); result != nil {
				data, _ = json.Marshal(result)

			} else {
				data, _ = json.Marshal(&AckJsonResponse{Message: fmt.Sprintf("post with id: %v does not exist", intId), Status: http.StatusNotFound})
...
func handleGetCommentsByPostId(svc *RestApiService) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//	]'
		id := utils.GetFirstParam("/api/get/comments/(.*)", r.URL.Path)
		intId, _ := strconv.ParseUint(id, 10, 64)
		results := svc.commentRepository.GetAllByPostId(intId)
		w.Header().Set("Content-Type", "application/json")
